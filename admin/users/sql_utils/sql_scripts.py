...
Role={
    ...
    "Get":"SELECT * FROM ROLE WHERE ID = COALESCE({0}, ID) AND NAME = COALESCE('{1}', NAME)",
...
User={
    ...
    "List":"SELECT USERS.ID id, USERS.USERNAME username, USERS.FIRSTNAME firstName, USERS.LASTNAME lastName, ROLE.NAME role, CRM.NAME crmName FROM USERS JOIN ROLE ON USERS.ROLE_ID = ROLE.ID JOIN CRM ON USERS.CRM_ID = CRM.ID",
    "Get":"SELECT ID id, USERNAME username, ROLE_ID roleId, CRM_ID crmId, FIRSTNAME firstName, LASTNAME lastName, PHONENUMBER phoneNumber, MOBILEPHONENUMBER mobilePhoneNumber, CREATEDDATE createdDate, UPDATEDDATE updatedDate FROM USERS WHERE ID = '{0}'",
    "Update":"UPDATE USERS SET ROLE_ID = '{0}', CRM_ID = '{1}', FIRSTNAME = '{2}', LASTNAME = '{3}', PHONENUMBER = '{4}', MOBILEPHONENUMBER = '{5}' WHERE ID = '{6}'",
